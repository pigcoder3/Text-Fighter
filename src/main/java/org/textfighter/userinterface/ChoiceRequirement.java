...
public class ChoiceRequirement {
    ...
    public boolean invokeRequirement() {
        try { return((boolean)method.invoke(arguments)); } catch (IllegalAccessException | InvocationTargetException e) { e.printStackTrace(); }
        return false;
    }

    public ChoiceRequirement(String method, ArrayList<String> arguments, ArrayList<Class> argumentTypes, String clazz) {
        ...
        for (int i=0; i<arguments.size(); i++) { if(parameterTypes[i].equals(Integer.class)) {this.arguments.add(Integer.parseInt((String)arguments.get(i)));} }
    }
